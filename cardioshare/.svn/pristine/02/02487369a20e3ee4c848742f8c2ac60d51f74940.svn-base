 ...
 window.SADI.init = function($) {
  if (!window.SADI.bookmarkletClicked) {
   (function() {
    ...
    // override the real methods we want to call with ones that fetch static data...
    PHL.sparql = function(endpoint, query, callback) {
        var local = callback;
        var global = (function(){return this;}).call();
            global.callback = function(data) {
                local(PHL.convert_bindings(data.results.bindings));
            };
        $.ajax({
                 url : "${deploy.root}/dummy/sparql.js",
            dataType : "jsonp"
        });
    };
    // main method...
